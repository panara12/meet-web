...
import { Eye, Download, FileText, Image, Calendar, Loader2, AlertCircle, RefreshCw, ChevronDown } from 'lucide-react';
        ...
        <div className="sm:hidden px-4 py-3 border-b border-gray-200">
          ...
          {/* <div className="mt-3 flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900">{currentDayInfo?.name}</p>
                {isToday && (
                  <p className="text-xs text-blue-600">Today's files</p>
                )}
              </div>
            </div>
            <div className="text-right">
              ...
              <p className="text-xs text-blue-600">
                {currentDayFiles.length === 1 ? 'file' : 'files'}
              </p>
            </div>
          </div> */}
        </div>
          ...
          {currentDayFiles.length > 0 ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-start sm:items-center gap-2 sm:gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-medium text-sm sm:text-base text-gray-900 truncate">{file.name}</h3>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-[10px] sm:text-xs text-gray-500 mt-1">
                          <span className="truncate">Uploaded by {file.uploadedBy}</span>
                          <span className="hidden sm:inline">•</span>
                          <span className="truncate">{file.uploadedAt}</span>
                        </div>
                        {file.description && (
                          <div className="text-[10px] sm:text-xs text-gray-600 mt-2 p-2 bg-gray-50 rounded border border-gray-200">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-0 sm:ml-4">
                      <button
                        ...
                        className="flex-1 sm:flex-initial flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
                        title="View file"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 sm:flex-initial flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-white bg-[#1E3986] hover:bg-[#162d73] rounded-md transition-colors"
                        title="Download file"
                      >
            ...
            <div className="text-center py-8 sm:py-12">
              ...
              {/* <p className="text-sm text-gray-500 px-4">
                Admin hasn't uploaded any files for {currentDayInfo?.name} this week yet.
              </p>
              <p className="text-xs sm:text-sm text-gray-400 mt-2 px-4">
                Files reset every Sunday at the start of a new week
              </p> */}
            </div>
          )}
        </div>
